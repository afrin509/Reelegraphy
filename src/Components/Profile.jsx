import { AppBar, Avatar, Card, Divider, IconButton, makeStyles, Toolbar, Typography } from '@material-ui/core';
    ...
    return (
        ...
        <div>
            ...
            <AppBar position="static" style={{backgroundColor:'white'}}>
                ...
                <Link to="/" ><HomeIcon style={{color:"gray"}}>
                    </HomeIcon></Link>
                <IconButton onClick={handleLogout} disabled={loading}><ExitToAppIcon>
                    </ExitToAppIcon></IconButton>
                <Avatar src={user.profileUrl} alt="profile" className={classes.small}></Avatar>
                </Toolbar>
            </AppBar>
            </div> 
            <div className={classes.flex}>
                <div className={classes.profileDiv}>
                    <Avatar src={user.profileUrl} alt="profile" className={classes.profile}></Avatar>
                </div>
                <div className={classes.profileDetails}>
                    <Typography style={{fontSize:"2rem",textAlign:"left"}}>{user.username}</Typography>
                    <Typography style={{fontSize:"1.2rem"}}>Number of Reels : {user.reels.length}</Typography>
                    <Typography style={{fontSize:"1.2rem"}}>Email : {user.email}</Typography>
                </div>
            </div>
            <Divider variant="middle"></Divider>
            <div className={classes.reels}>
            {videos.map((videoObj, idx) => {
                        return <Card variant="outlined" style={{borderRadius:"8px",height:"60vh",marginLeft:"1rem",marginRight:"1rem"}} key={videoObj.rid}>
                            <video muted={true}  autoPlay loop style={{height:"100%"}}>
                                    <source src={videoObj.videoUrl}></source>
                                </video>
                        </Card>
                    })}
            </div>
